...
#pragma warning restore CA2227 // 集合属性应为只读
    ...
    {
        private readonly List<JavScraperConfigItem> _items = new();
        ...
        public void Add(JavScraperConfigItem item)
        {
            ...
            {
                if (_items[i].Name.Equals(item.Name, StringComparison.Ordinal))
                {
                    _items.RemoveAt(i);
                }
            }
        ...
        public bool Contains(JavScraperConfigItem item) => _items.Contains(item);

        public void CopyTo(JavScraperConfigItem[] array, int arrayIndex) => _items.CopyTo(array, arrayIndex);

        public IEnumerator<JavScraperConfigItem> GetEnumerator() => _items.GetEnumerator();

        public bool Remove(JavScraperConfigItem item) => _items.Remove(item);
