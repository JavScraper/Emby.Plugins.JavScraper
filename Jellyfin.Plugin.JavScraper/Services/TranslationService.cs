...
namespace Jellyfin.Plugin.JavScraper.Services
{
    ...
    public class TranslationService
    {
        ...
        /// 翻译
        /// </summary>
        public TranslationService(ILoggerFactory loggerFactory, ApplicationDbContext applicationDbContext)
        {
        ...
        public async Task<string> Translate(string src)
        {
            ...
            using (await _locker.WaitAsync(hash).ConfigureAwait(false))
            {
                try
                {
                    var item = _applicationDbContext.Translations.FindOne(o => o.Hash == hash && o.Lang == lang);
                    if (item != null)
                    {
                        return item.Dst;
                    }
                    ...
                    if (fanyi_result?.TransResult?.Any() == true)
                    {
                        ...
                        if (string.IsNullOrWhiteSpace(dst) == false)
                        {
                            ...
                            _applicationDbContext.Translations.Insert(item);
                            return dst;
                        }
                    }
